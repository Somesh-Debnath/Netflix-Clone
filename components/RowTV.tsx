...
function RowTV({title,tvs}:Props) {
  ...
  return (
    <div className="h-40 space-y-0.5 md:space-y-2">
        ...
        <div className="group relative">
            <ChevronLeftIcon className={`absolute top-0 bottom-0 z-40 left-2 
            ...
            md:space-x-2.5 md:px-0 md:py-2 scrollbar-hide">
               {tvs.map((tv)=>(
                    <TVThumbnail key={tv.id} tv={tv}/>
               ))}
               
            </div>
            <ChevronRightIcon className='absolute top-0 bottom-0 right-2 z-40
            m-auto h-9 w-9 cursor-pointer opacity-0 hover:scale-125 
